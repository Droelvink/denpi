...
Optionally `npm link` to get a global `clover` command.
...
| `/btw <question>` | quick side question, even while clover is working — sees the whole conversation but uses no tools and leaves no trace in context |
...
| *typing while clover works* | sends a message the model sees at its next step — steer without stopping |
...
at — `/searxng <url>` in-session (persisted to `~/.clover/settings.json`), or `--searxng <url>` /
`CLOVER_SEARXNG` to override per launch. Pasting the search page URL is fine — a trailing
...
- **CLOVER.md** — put one in the workspace root and its content is appended to the
...
- **Skills** — folders with a `SKILL.md` playbook (YAML frontmatter: `name`,
  `description`). Global: `~/.clover/skills/<name>/`, per-workspace:
  `.clover/skills/<name>/` (workspace wins on collisions). clover lists them in the
...
clover prompts only when a call reaches outside the workspace (file paths, absolute
...
commands stream their last output lines live under the activity spinner, and code blocks
in responses are syntax-highlighted.

File tools are sandboxed to the workspace (the directory clover was started in):
...
src/
  ...
  config.ts          config from flags + env (CLOVER_URL, CLOVER_MODEL)
  theme.ts           colors, glyphs, the spinner
  ...
  ui/                Ink components: transcript, markdown, activity spinner, approval
```
